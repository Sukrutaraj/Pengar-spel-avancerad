...
function spawnEnemy() {
    ...
    // Filter moneyTypes based on random chance
    const filtered = moneyTypes.filter(m => Math.random() * 100 < m.chance);

    if(filtered.length === 0) return; // no spawn if none passed

    // Random choice among allowed types
...
function dynamicSpawn() {
    if(gameOver) return;
    let spawnCount = 1 + Math.floor(score / 500); // fler fiender vid högre saldo
...
function update() {
    ...
    if(score < 0 && !gameOver) {
        gameOver = true;
        gameOverSound.play();
        setTimeout(()=>alert("GAME OVER! Du har gått under 0 kr."), 100);
    }
}
